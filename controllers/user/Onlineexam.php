...
class Onlineexam extends MY_Controller {
    ...
    public function index() {
		...
		$student=get_user_by_role(STUDENT,$user_id);
		$student_id=$student->id;
		//print_r($this->session->userdata()); exit;
        $onlineexam = $this->onlineexam_model->getStudentexam($student_id);
        $data['onlineexam'] = $onlineexam;
		$this->layout->title($this->lang->line('online')." ".$this->lang->line('exam') . ' | ' . SMS);		
        $this->layout->view('user/onlineexam/onlineexamlist', $data);       
    }
    ...
    public function view($id) {
       ...
       // $this->session->set_userdata('top_menu', 'Onlineexam');
		...
		$student=get_user_by_role(STUDENT,$user_id);
		$student_id=$student->id;
        $online_exam_validate = $this->onlineexam_model->examstudentsID($student_id, $id);		
        ...
        // $data['online_exam_validate'] = $online_exam_validate;
	$this->layout->title($this->lang->line('online')." ".$this->lang->line('exam') . ' | ' . SMS);
        $this->layout->view('user/onlineexam/view', $data);   
    ...
    public function getExamForm() {
		...
		$student=get_user_by_role(STUDENT,$user_id);
		$student_id=$student->id;
        $onlineexam_student = $this->onlineexam_model->examstudentsID($student_id, $exam->id);
        ...
        $questionOpt = $this->getQuesOption();
        $data['questionOpt'] = $questionOpt;
