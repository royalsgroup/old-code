...
{	
  public $sch_setting_detail = array();
    public function __construct()
    {
		...
		$this->load->model('onlineexamquestion_model'); 
    ...
    {			
       ...
       // $this->session->set_userdata('top_menu','Online_Examinations');
        //$this->session->set_userdata('sub_menu','Online_Examinations/Onlineexam');
		 $this->data['onlineexam'] = $this->onlineexam_model->get_exam_list($school_id);  
		...
		$this->data['list'] = TRUE;
		$this->layout->title($this->lang->line('online')." ".$this->lang->line('exam') . ' | ' . SMS);
        $this->layout->view('onlineexam/index', $this->data);
    }
    ...
    {       
		...
		$condition = array();
        $condition['school_id'] = $onlineexam->school_id;		
		$data['classlist'] = $this->classes->get_class_list($condition['school_id']);
       ...
       // $class              = $this->class_model->get();
		...
		$this->layout->title($this->lang->line('online')." ".$this->lang->line('exam'). ' | ' . SMS);
        $this->layout->view('onlineexam/assign', $data);       
    }

    public function addstudent()
    {
        $this->form_validation->set_rules('onlineexam_id', $this->lang->line('exam')." ".$this->lang->line('id'), 'required|trim');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($resultlist)) {
                ...
                foreach ($resultlist as $each_student_key => $each_student_value) {
                    if ($each_student_value['onlineexam_student_id'] != 0) {
                        $all_students[] = $each_student_value['onlineexam_student_id'];
                    }
            ...
            $array = array('status' => 'success', 'error' => '', 'message' => 'Record saved');
            echo json_encode($array);
        }
    }
    ...
    public function searchQuestionByExamID()
    {
        ...
        if (isset($page)) {
            ...
            if ($count) {
                $pag_content = $this->load->view('onlineexam/_searchQuestionByExamID', $data, true);
	...
	if ($_POST) {
		$this->_prepare_exam_validation();		
            if ($this->form_validation->run() === TRUE) {				
                ...
                $insert_id = $this->onlineexam_model->insert('onlineexam', $data);
                if ($insert_id) {
                  ...
                  //  $this->__create_default_section($insert_id);
                    redirect('onlineexam/index/'.$data['school_id']);
                } else {
	 ...
	 $this->data['onlineexam'] = $this->onlineexam_model->get_exam_list($school_id);  
        ...
        $this->data['subjectlist']  = $subject_result;      
		$this->data['list'] = TRUE;
		$this->layout->title($this->lang->line('Online Examinations') . ' | ' . SMS);
        $this->layout->view('onlineexam/index', $this->data);
        //echo json_encode($array);
    }
	public function edit($id = null) {                     
		...
		if ($_POST) {			
            ...
            if ($this->form_validation->run() === TRUE) {				
                $data = $this->_get_posted_exam_data();						
                $updated = $this->onlineexam_model->update('onlineexam', $data, array('id' => $this->input->post('id')));
                ...
                $this->data['exam'] = $this->onlineexam_model->get_single('onlineexam', array('id' => $this->input->post('id')));
            }
        }
        
        if ($id) {
            $this->data['exam'] = $this->onlineexam_model->get_single('onlineexam', array('id' => $id));
        ...
        $this->data['edit'] = TRUE;       
        $this->layout->title($this->lang->line('edit'). ' ' . $this->lang->line('exam'). ' | ' . SMS);
        $this->layout->view('onlineexam/index', $this->data);
    ...
    {		
		...
		$this->layout->title($this->lang->line('online')." ".$this->lang->line('exam') . ' | ' . SMS);
        $this->layout->view('onlineexam/results', $this->data);
    }
    ...
    public function getRecord($id)
    {
	 ...
	 public function delete($id = null) {
        ...
        $exam = $this->onlineexam_model->get_single('onlineexam', array('id' => $id));
        
        if ($this->onlineexam_model->delete('onlineexam', array('id' => $id))) {
        ...
        redirect('onlineexam/index/'.$exam->school_id);
    }   
    public function questionAdd()
    {

        $this->form_validation->set_rules('question_id', $this->lang->line('exam'), 'trim|required');
        $this->form_validation->set_rules('onlineexam_id', $this->lang->line('attempt'), 'trim|required');
	...
	private function _get_posted_exam_data() {
        ...
        $items[] = 'time_to';
		$items[] = 'duration';        
        $items[] = 'passing_percentage';
		$items[] = 'description';        
        $items[] = 'publish_result';		               
        $data = elements($items, $_POST);        
        $data['is_active']= 0;
		$data['exam_from']=date('Y-m-d',strtotime($_POST['exam_from']));
		$data['exam_to']=date('Y-m-d',strtotime($_POST['exam_to']));
            $data['publish_result'] = 0;
            if (isset($_POST['is_active'])) {
                $data['is_active'] = 1;
            }
            if (isset($_POST['publish_result'])) {
                $data['publish_result'] = 1;
            }
	...
	private function _prepare_exam_validation() {
        ...
        $this->form_validation->set_rules('exam_from', $this->lang->line('exam'). ' '.$this->lang->line('from'), 'trim|required');     
        $this->form_validation->set_rules('exam_to', $this->lang->line('exam').' '.$this->lang->line('to'), 'trim|required');
		$this->form_validation->set_rules('passing_percentage', $this->lang->line('passing'). ' '.$this->lang->line('percentage'), 'trim|required');  
