...
class Idsetting extends MY_Controller {
    ...
    * ********************************************************** */  
    ...
    private function _get_posted_setting_data() {
		...
		if(isset($_FILES['background_image']['name'])){  
            $data['background_image'] = $this->_upload_background();
        }
   ...
   * ********************************************************** */
	...
	private function _upload_background() {
        ...
        $background_type = $_FILES['background_image']['type'];
        $return_background = '';
        if ($background != "") {
            if ($background_type == 'image/jpeg' || $background_type == 'image/pjpeg' ||
                ...
                $destination = 'assets/uploads/idcard/';

                $file_type = explode(".", $background);
                $extension = strtolower($file_type[count($file_type) - 1]);
                $background_path = 'idcard-' . time() . '-sms.' . $extension;

                move_uploaded_file($_FILES['background_image']['tmp_name'], $destination . $background_path);
