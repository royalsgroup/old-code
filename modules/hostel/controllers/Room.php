...
class Room extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($school_id = null) {
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN && $this->session->userdata('dadmin') != 1){            
            ...
            if(!$school_id) $school_id =  $this->session->userdata('school_id');
        }        
        $this->data['rooms'] = $this->room->get_room_list($school_id);
        $this->data['filter_school_id'] = $school_id;
        $this->data['schools'] = $this->schools;
        $academic_year=$this->room->get_single('academic_years',array('school_id'=>$school_id,'is_running'=>1));	
        $this->data['academic_year_id'] = $academic_year->id ?? 0; 
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        $academic_year=$this->room->get_single('academic_years',array('school_id'=>$this->data['room']->school_id,'is_running'=>1));	
        $this->data['academic_year_id'] = $academic_year->id ?? 0; 
    ...
    * ********************************************************** */
    private function _get_posted_room_data() {
        ...
        $academic_year=$this->room->get_single('academic_years',array('school_id'=>$data['school_id'],'is_running'=>1));	
        $iAcademicYearID = $academic_year->id ?? 0; 
        if ($this->input->post('id')) {
            $room_data = $this->room->get_single('rooms',array('id'=>$this->input->post('id')));
            $yearly_room_rent = $room_data->yearly_room_rent ? json_decode($room_data->yearly_room_rent,true) : array();
            if($iAcademicYearID)
            {
                $yearly_room_rent[$iAcademicYearID] = $data['cost'];
                unset($data['cost']);
            }
            $data['yearly_room_rent'] = json_encode($yearly_room_rent); 	
            $data['modified_at'] = date('Y-m-d H:i:s');
            $data['modified_by'] = logged_in_user_id();
        } else {
            $data['yearly_room_rent'] = json_encode(array($iAcademicYearID=>$data['cost']));
            $data['status'] = 1;
