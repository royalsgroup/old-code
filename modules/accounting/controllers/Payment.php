...
class Payment extends My_Controller {
    ...
    * ********************************************************** */
    public function index($invoice_id = null) {
        ...
        if($invoice->previous_due_amount) $due_amount      = $due_amount + $invoice->previous_due_amount;
    ...
    * ********************************************************** */
    public function paid($invoice_id) {
        ...
        if ($_POST) {
            $invoice         = $this->payment->get_invoice_amount($invoice_id);      
            $_POST['amount']  = $invoice->net_amount;
					...
					$feetype_id=$invoice_detail->income_head_id;
						...
						if(isset($transaction['voucher_id'])){
							...
							if ($transaction_id) {
                                ...
                                $invoice         = $this->payment->get_single_invoice($invoice_id);  
    ...
    * ********************************************************** */
    private function _prepare_payment_validation() {
        ...
        $this->form_validation->set_rules('amount', $this->lang->line('amount'), 'trim');   
