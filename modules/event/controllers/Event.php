  ...
  /*****************Function _send_message_notification**********************************
    ...
    * ********************************************************** */
    public function _send_message_notification($message_data = null) {
        ...
        $school                     = $this->event->get_school_by_id($message_data['school_id']);
        $users                      = $this->event->get_event_users($message_data['school_id'],$message_data['role_id']);
        if(!$message_data['role_id'])
        {
            $super_admins =  $this->event->get_event_users($message_data['school_id'],SUPER_ADMIN);
            $users        = array_merge($users,$super_admins);
        }
        $event                     = $this->event->get_single_event($message_data['id']);  
        $data['academic_year_id']   = $school ->academic_year_id;
        $data['subject']            = $this->lang->line('event')." Notification";   
        ...
        foreach ($users as $obj) {
            ...
            if($obj->id != '' && logged_in_user_id()!= $obj->id){   
                $message = '<b>'.$this->lang->line('event_details').'</b>';
                $message .= '<br/><br/>';            
                $message .= $this->lang->line('title').': '.$event->title;     
                $message .= '<br/>';
                $message .= $this->lang->line('event_place').': '.$event->event_place;
                $message .= '<br/>';
                $message .= $this->lang->line('from_date').': '.$event->event_from.", ";
                $message .= $this->lang->line('to_date').':'.$event->event_to;
                if($event->note)
                {
                    $message .= '<br/>';
                    $message .= $this->lang->line('note').' :'.$event->note;
                    $message .= '<br/>';
                }
                $data['body'] = $message;
