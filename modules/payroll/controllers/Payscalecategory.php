...
class Payscalecategory extends MY_Controller {
    ...
    public function insert_defualt($school_id =null)
    {
        ...
        $this->data['grades'] = $this->grade->get_cat_list($school_id);   
        if(count($this->data['grades']) >0)
        {
            error($this->lang->line('insert_faild'));
            redirect('payroll/payscalecategory/index');
        }
        $this->grade->insert_default($school_id);
        redirect('payroll/payscalecategory/index'.'/'.$school_id);
    }
    public function fix_defualt($school_id = null)
    {
        if(!$school_id)
        {
            error($this->lang->line('select_school'));
        ...
        $this->grade->fix_default($school_id);
...
//$this->form_validation->set_rules('credit_ledger_id', $this->lang->line('credit_ledger_id'), 'trim|required');
   ...
   public function grade_name(){       
    ...
    private function _get_posted_grade_data() {
        ...
        $items[] = 'change_default_debit_ledger';
        $items[] = 'archive';
        ...
        if($data['is_deduction_type'] == 'FALSE')
        {
            $data['credit_ledger_id']  = 0;
        }
       
        if(!$data['amount']) $data['amount']=0;
        if(!$data['percentage']) $data['percentage']=0;
        if(!$data['max_amount_possible']) $data['max_amount_possible']=0;
        if(!$data['change_default_debit_ledger']) $data['change_default_debit_ledger']=0;
        if(!$data['archive']) $data['archive']=0;
        
        $data['debit_ledger_id']  = 0;
