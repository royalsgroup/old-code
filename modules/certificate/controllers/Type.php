...
class Type extends MY_Controller {
    ...
    * ********************************************************** */
    private function _prepare_type_validation() {
        ...
        $this->form_validation->set_message('prev_background', 'The Image file size shoud not exceed 1MB!');
        ...
        $this->form_validation->set_rules('prev_background', 'prev_background', 'callback_validate_prev_background_size');
    ...
    * ********************************************************** */ 
    ...
    public function validate_prev_background_size() {

    
        if ((!isset($_FILES['prev_background'])) || $_FILES['prev_background']['size'] == 0) 
        {
            return true;
        }
        else if (isset($_FILES['prev_background']) && $_FILES['prev_background']['size'] != 0) {
            if(filesize($_FILES['prev_background']['tmp_name']) >= 1048676) {
                return false;
            }
        }
        return true;
    ...
    private function _upload_background() {
        ...
        if ($background != "") {
            if ($background_type == 'image/jpeg' || $background_type == 'image/pjpeg' ||
                ...
                if($converted_file = webpConverter($destination . $background_path))
                {
                    $background_path = get_filename($converted_file);
                }
