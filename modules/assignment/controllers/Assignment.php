...
class Assignment extends MY_Controller {
    ...
    * ********************************************************** */
    ...
    public function _send_message_notification($assignment_data = null) {
 
        $data                       = array("");
        $data['school_id']          = $assignment_data['school_id'];
        ...
        $students                   = $this->assignment->get_student_list($assignment_data['school_id'], $class_id, $section_id, $assignment_data['academic_year_id'] );
        $assignment                    = $this->assignment->get_single_assignment($assignment_data['id']);  
        $data['academic_year_id']   = $assignment_data['academic_year_id'];

        $data['subject']            = "New ".$assignment->subject." ".$this->lang->line('assignment');   
        ...
        foreach ($students as $obj) {
            ...
            if($obj->user_id != ''){                    
                ...
                $message .= '<br/>';
                $message .= $this->lang->line('assignment_details');
                $message .= '<br/><br/>';
                ...
                $message .= '<br/>';
                $message .= $this->lang->line('deadline'). ': ' . $assignment->deadline;
                if($lecture->note)
                {
                    $message .= '<br/>';
