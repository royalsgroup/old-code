...
class Student extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($class_id = null) {
	 ...
	 public function alumni($class_id = null) {
        ...
        if($school_id){
            $this->data['students'] = $this->student->get_alumnistudent_list($class_id, $school_id, $school->academic_year_id);
        }
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
					...
					if(isset($_POST['alumni'])){
						redirect('student/alumni/'.$this->input->post('class_id'));
					}
					else{
						redirect('student/index/'.$this->input->post('class_id'));
					}
 ...
 $this->form_validation->set_rules('student_code', $this->lang->line('student_code'), 'trim|required|callback_studentcode');
    ...
    * ********************************************************** */ 
    public function username() {
	 ...
	 public function studentcode() {
        if ($this->input->post('id') == '') {
            $emp = $this->student->duplicate_check_student_code($this->input->post('student_code'));
            if ($emp) {
                $this->form_validation->set_message('student_code', $this->lang->line('already_exist'));
                return FALSE;
            } else {
                return TRUE;
            }
        } else if ($this->input->post('id') != '') {
            $emp = $this->student->duplicate_check_student_code($this->input->post('student_code'), $this->input->post('id'));
            if ($emp) {
                $this->form_validation->set_message('student_code', $this->lang->line('already_exist'));
                return FALSE;
            } else {
