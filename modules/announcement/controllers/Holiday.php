...
class Holiday extends MY_Controller {
    ...
    * ********************************************************** */
    ...
    public function _send_message_notification($message_data = null) {
        ...
        $school                     = $this->holiday->get_school_by_id($message_data['school_id']);
        $users                      = $this->holiday->get_holiday_users($message_data['school_id']);
        $holiday                     = $this->holiday->get_single_holiday($message_data['id']);  
        $data['academic_year_id']   = $school ->academic_year_id;

        $data['subject']            = $this->lang->line('holiday')." Notification";   
        $data['sender_id']          = logged_in_user_id();
        $data['sender_role_id']     = $this->session->userdata('role_id');
        foreach ($users as $obj) {

            if($obj->id != '' ){                    
                $message = $this->lang->line('holiday_details');
                $message .= '<br/><br/>';
                $message .=  $this->lang->line('title').' : '.$holiday->title;
                $message .= '<br/>';
                $message .=  $this->lang->line('from_date').' : '.$holiday->date_from;
                $message .= '<br/>';
                $message .=  $this->lang->line('to_date').' : '.$holiday->date_to;
                if($holiday->note)
                {
                    $message .= '<br/>';
                    $message .=  $this->lang->line('note').' : '.$holiday->note;
                    $message .= '<br/>';
                }
