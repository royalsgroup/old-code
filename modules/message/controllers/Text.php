...
class Text extends MY_Controller {
    ...
    * ********************************************************** */
    private function _send_sms($data) {
        $responses = [];
        $receivers = '';
        if($this->input->post('template') == "6" )
        {
            $users = $this->sms->get_result_user_list($data['school_id'], $this->input->post('class_id'),$this->input->post('receiver_id'));
        }
        else
        {
        ...
        foreach ($users as $obj) {
            ...
            if($this->input->post('template') == "6")
            {
                $variables = array(
                    ...
                    'sr_no'=>  $obj->admission_no,
                    'max_mark'=>  $obj->max_marks,
                    'obtained_mark'=>  $obj->obtain_mark,
                    'percentage'=>  $obj->percentage,
                    'grade'=>  $obj->grade,
                    'result'=>  $obj->result,
                    'class_name'=>  $obj->class_name,
                    'admin_name'=> FROM_NAME
                );
                $message = replace_variables($variables, $data['body']);
            }
            else
            {
            ...
            if ($obj->phone != '') {
                $phone = '+' . $obj->phone;
                $responses[$obj->id][] = $this->_send($data['sms_gateway'], $phone, $message);
            }
        }
    ...
    * ********************************************************** */
    public function _send($sms_gateway, $phone, $message) {
        ...
        if ($sms_gateway == "clicktell") {
            ...
            // {
            //     $error_code = isset($response->errors) && !empty($response->errors) ? $response->errors['0']->code : "";
            //     $error_message = isset($response->errors) && !empty($response->errors) ? $response->errors['0']->message : "Error";
