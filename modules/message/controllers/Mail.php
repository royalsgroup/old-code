    ...
    * ********************************************************** */
    private function _send_email($data) {
        ...
        if(empty($email_setting)){
            $email_setting = $this->mail->get_single('email_settings', array('status' => 1, 'school_id'=>0));   
        } 
        ...
        $receivers = '';
        if($this->input->post('template') == "89")
        {
            $users = $this->mail->get_result_user_list($data['school_id'], $this->input->post('class_id'));
        }
        else
        {
        ...
        foreach ($users as $obj) {            
            ...
            if($obj->email != '' && $obj->result_id){ 
                ...
                if(!empty($email_setting) && $email_setting->mail_protocol == 'smtp'){
                    ...
                    $result = mail(trim($obj->email), $data['subject'], $message, $headers);
                    
                    // debug_a($result,"|$obj->email|  $data[subject]",1);
