...
class Route extends MY_Controller {
    ...
    * ***********************************************************/
    public function add() {
        ...
        $this->data['schools'] = $this->schools;
        $academic_year=$this->route->get_single('academic_years',array('school_id'=>$condition['school_id'],'is_running'=>1));	
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        $this->data['schools'] = $this->schools;
        $academic_year=$this->route->get_single('academic_years',array('school_id'=>$this->data['route']->school_id,'is_running'=>1));	
        $this->data['academic_year_id'] = $academic_year->id ?? 0; 
        $this->data['edit'] = TRUE;
    ...
    private function _save_stop($route_id){
        ...
        foreach($this->input->post('stop_name') as $key=>$value){
            ...
            if($value){
                ...
                $academic_year=$this->route->get_single('academic_years',array('school_id'=>$school_id,'is_running'=>1));	
                $iAcademicYearID = $academic_year->id ?? 0; 
                if ($this->input->post('id') && $exist) {                
                    $data['modified_at'] = date('Y-m-d H:i:s');
                    $data['modified_by'] = logged_in_user_id(); 
                    $yearly_stop_fares = $exist->yearly_stop_fare ? json_decode($exist->yearly_stop_fare,true) : array();
                    if($iAcademicYearID)
                    {
                        $yearly_stop_fares[$iAcademicYearID] = @$_POST['stop_fare'][$key];
                    }
                    $data['yearly_stop_fare'] = json_encode($yearly_stop_fares); 
