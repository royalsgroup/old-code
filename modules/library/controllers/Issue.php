...
class Issue extends MY_Controller {
    ...
    * ********************************************************** */
    public function _send_message_notification($message_data = null) {
        ...
        $book                       = $message_data['book'];
        $issue_register             = $this->book->get_single_issue_detail($message_data['id']);  

        $data['academic_year_id']   = $message_data['academic_year_id'];
        $profile                    = get_user_by_role($issue_register->user_role_id, $issue_register->user_id);
        
        $data['subject']            = $this->lang->line('new_book_issued');   
        $data['sender_id']          = logged_in_user_id();
        $data['sender_role_id']     = $this->session->userdata('role_id');
             
        $message = $this->lang->line('hi'). ' '. $profile->name.',';
        $message .= '<br/>';
        $message .= $this->lang->line('following_is_your_book_issue_details');
        $message .= '<br/><br/>';
        $message .= $this->lang->line('book').': ' . $book->title;
        $message .= '<br/>';
        $message .= $this->lang->line('author').': ' . $book->author;
        $message .= '<br/>';
        $message .= $this->lang->line('due_date').': ' . $issue_register->due_date;
        $message .= '<br/>';
        $message .= $this->lang->line('issue_date').': ' . $issue_register->issue_date;
       
        $message .= '<br/><br/>';

        $message .= $this->lang->line('thank_you').'<br/>';
        $data['body'] = $message;
        $data['receiver_id'] = $issue_register->user_id;
        $data['receiver_role_id'] = $issue_register->user_role_id;
