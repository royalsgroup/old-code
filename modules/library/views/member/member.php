<div class="row">
                            ...
                            <table id="datatable-keytable" class="datatable-responsive table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th><?php echo $this->lang->line('action'); if(has_permission(DELETE, 'library', 'member')){?> <button class="btn btn-xs btn-success " type="button" id="bulk_remove_button" >Bulk Remove</button> <?php } ?></th>                                            
                                    </tr>
                                </thead>
                                <tbody>   
 ...
 <script type="text/javascript">
    $(document).ready(function() {
        ...
        if($(this).is(':checked')){
                $('.library_member').prop('checked', true);
            }else{
                $('.library_member').prop('checked', false);
            }
        })
      ...
      'serverMethod': 'post',
      'ajax': {
          'url':'<?php echo site_url("library/member/get_members_list"); ?>',
    ...
    $('#bulk_remove_button').on('click', function(){
        var selectedID = [];
        $(".library_member[type='checkbox']").each (function () {
            if($(this).is(":checked")){
        ...
        else
        {
            ...
            if(selectedID.length && confirm_bulk_pass && !error)
            {
                $.ajax({       
                type   : "POST",
                url    : "<?php echo site_url('library/member/remove_bulk'); ?>",
