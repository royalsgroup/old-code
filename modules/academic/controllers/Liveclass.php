...
class Liveclass extends MY_Controller
{
    ...
    public function index($class_id = null)
    {
        ...
        if ($this->session->userdata('role_id') != SUPER_ADMIN && $this->session->userdata('dadmin') != 1) {
            ...
            //$this->data['classes'] = $this->liveclass->get_class_by_session('classes', $condition['school_id']);
        ...
        $this->data['class_list'] = $this->classes->get_class_list( $condition['school_id']);
           // echo $this->db->last_query();
            // die();
        //$this->data['class_list'] = $this->liveclass->get_class_by_session('classes', $condition['school_id']);
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if($duplicate  == 1)
                {
                    ...
                    if ($insert_id) {
                        ...
                        success($this->lang->line('insert_success')."".$insert_id );
                        redirect('academic/liveclass/index/' . $data['class_id']);
                    } else {
                        error($this->lang->line('insert_failed'));
                        redirect('academic/liveclass/add/' . $data['class_id']);
    ...
    private function _prepare_live_class_validation()
    {
        ...
        if ($this->input->post('class_type') == 'zoom' || $this->input->post('class_type') == 'google_meet') {
            $this->form_validation->set_rules('meeting_link', $this->lang->line('meeting_link'), 'trim|required');
    ...
    private function _get_posted_live_class_data()
    {
        ...
        if ($this->session->userdata('role_id') == TEACHER) {
            ...
            $data['teacher_id'] =$this->session->userdata('profile_id') ;
        }
        ...
        if ($this->input->post('id') && !$this->input->post('duplicate')) {
    ...
    * @return          : null 
    * ********************************************************** */
    public function _send_message_notification($live_data = null) {
 
        $data                       = array("");
        $data['school_id']          = $live_data['school_id'];
        $school = $this->liveclass->get_school_by_id($data['school_id']);
        $students                   = $this->liveclass->get_student_list($live_data['school_id'], $live_data['class_id'], $live_data['section_id'], $school->academic_year_id);
        $live_class                 = $this->liveclass->get_single_live_class($live_data['id']);  
        $data['academic_year_id']   = $school->academic_year_id;

        $data['subject']            = $live_class->subject;  
        $data['sender_id']          = $live_class->t_user_id;
