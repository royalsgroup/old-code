                ...
                <div class="" data-example-id="togglable-tabs">
                    ...
                    <ul  class="nav nav-tabs bordered  no-print">
                        ...
                        <li class="li-class-list">
                            <?php if($this->session->userdata('role_id') != SUPER_ADMIN && $this->session->userdata('role_id') != DISTRICT_ADMIN && !$this->session->userdata('role_id')){  ?> 
                            ...
                            <div class="x_content"> 
                               <?php echo form_open(site_url('academic/routine/add'), array('name' => 'add', 'id' => 'add', 'class'=>'form-horizontal form-label-left'), ''); ?>
                                ...
                                <div class="item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <select  class="form-control col-md-7 col-xs-12 day_selection" name="day" id="day" required="required">
                                            <option value="">--<?php echo $this->lang->line('select'); ?>--</option>
                                            <option value="-1"><?php echo $this->lang->line('multiple_days'); ?></option>
                                            <?php $types = get_week_days(); ?>
                                          
                                            <?php foreach($types as $key=>$value){ ?>
                                                <option value="<?php echo $key; ?>" <?php echo isset($post['day']) && $post['day'] == $key ?  'selected="selected"' : ''; ?>><?php echo $value; ?></option>
                                            <?php } ?>
                                        </select>
                                        <div class="help-block"><?php echo form_error('day'); ?></div>
                                        <div id="all-days-row" class="<?php echo  isset($post['day']) && $post['day'] == -1 ? '' : 'hidden' ?>">
                                            <?php foreach($types as $key=>$value){ ?>
                                                <input name="days[]" type="checkbox" value="<?php echo $key; ?>" > <?php echo $value; ?>
                                            <?php } ?>
                                        </div>
                                    </div>
                                </div>
...
$(document).on('change','.day_selection',function(e){
     var that = e.target;
     console.log("Dsd")
     if($(that).val() == "-1")
    {
        $('#all-days-row').removeClass('hidden');
    }
    else $('#all-days-row').addClass('hidden');
})
