    ...
    function __construct() {
     ...
     public function get_section_list($class_id = null, $school_id = null ){
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN && $this->session->userdata('role_id') != DISTRICT_ADMIN){
            $this->db->where("(S.school_id = ".$this->session->userdata('school_id')." or S.school_id = '0')"); //added by jsharookh as it was wrong 
            //$this->db->or_where('S.school_id','0');
        }
        
        if($school_id && $this->session->userdata('role_id') == SUPER_ADMIN){
            $this->db->where("(S.school_id = ".$school_id." or S.school_id = '0')");
