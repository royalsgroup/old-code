...
class About extends MY_Controller {
    ...
    * ********************************************************** */
    private function _prepare_about_validation() {
        ...
        $this->form_validation->set_rules('about_image', $this->lang->line('about').' '.$this->lang->line('image'), 'trim|callback_about_image|callback_image_size');
    }
    ...
    * ********************************************************** */ 
    ...
    public function image_size() 
    {
        ...
        else if (isset($_FILES['about_image']) && $_FILES['about_image']['size'] != 0) {
            if(filesize($_FILES['about_image']['tmp_name']) >= 1048676) {
                $this->form_validation->set_message('about_image', 'The Image file size shoud not exceed 1MB!');
                return false;
            }
        }
        return true;
    ...
    private function _upload_about_image() {
        ...
        if ($about_image_name != "") {
            if ($about_image_type == 'image/jpeg' || $about_image_type == 'image/pjpeg' ||
                ...
                if($converted_file = webpConverter($destination . $about_image_path))
                {
                    $about_image_path = get_filename($converted_file);
                }
