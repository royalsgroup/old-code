...
class Complain extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($school_id = null) {
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN && $this->session->userdata('dadmin') != 1){ 
            ...
            $this->data['classes'] = $this->complain->get_list('classes', $condition, '', '', '', 'id', 'ASC');
        }        
    ...
    * ********************************************************** */
	...
	public function generate_complain_no(){
		$school_id=$this->input->post('school_id');
		$complain_no = $this->complain->generate_complain_no($school_id);
		print $complain_no;
		exit;
	}
    ...
    * ********************************************************** */
    public function _send_message_notification($message_data = null) {
        ...
        $admins                     = $this->complain->get_reporting_users($message_data['school_id']);
        $super_admins               = $this->complain->get_reporting_users(null,null,SUPER_ADMIN);
        $source_users               = array_merge($admins,$super_admins);
        $complain                   = $this->complain->get_single_complain($message_data['id']);  
        $data['academic_year_id']   = $message_data['academic_year_id'];

        $data['subject']            = "New Complain";  
        $data['sender_id']          = logged_in_user_id();
        $data['sender_role_id']     = $this->session->userdata('role_id');

       
        $complain_user = get_user_by_role($complain->role_id, $complain->user_id);
        if($complain->user_id != ''){                    
            $message = $this->lang->line('hi'). ' '. $complain_user->name.',';
            $message .= '<br/>';
            $message .= $this->lang->line('recived_complaint_about_you');
            $message .= '<br/><br/>';
            $message .= $this->lang->line('complaint_details')." :";
            $message .= '<br/>';
            $message .= $this->lang->line('complain_type'). ': ' . $complain->type;
            $message .= '<br/>';
            $message .= $this->lang->line('complain'). ' No: ' . $complain->complain_no;
            $message .= '<br/>';
            $message .= $this->lang->line('complain_date'). ': ' . $complain->complain_date;
            $message .= '<br/>';
            $message .= $this->lang->line('description'). ': ' . $complain->description;
            $message .= '<br/><br/>';

            $message .= $this->lang->line('thank_you').'<br/>';
            $data['body'] = $message;
            $data['receiver_id'] = $complain->user_id;
            $data['receiver_role_id'] = $complain->role_id;
            $this->message->send_message($data);
        }
        // guardian message
        if($complain->role_id == STUDENT && $complain_user->g_user_id != ''){ 

            $message = $this->lang->line('hi'). ' '. $complain_user->guardian.',';
            $message .= '<br/>';
            $message .= $this->lang->line('recived_complaint_about_your_child');
            $message .= '<br/><br/>';
            $message .= $this->lang->line('complaint_details')." :";
            $message .= '<br/>';
            $message .= $this->lang->line('student').': ' . $complain_user->name;
            $message .= '<br/>';
            $message .= $this->lang->line('class').': ' . $complain_user->class_name;
            $message .= '<br/>';
            $message .= $this->lang->line('complain_type'). ': ' . $complain->type;
            $message .= '<br/>';
            $message .= $this->lang->line('complain'). ' No: ' . $complain->complain_no;
            ...
            $message .= $this->lang->line('thank_you').'<br/>';
            $data['body'] = $message;
            $data['receiver_id'] = $complain_user->g_user_id;
        ...
        {
            $sourse_user = get_user_by_role($obj->role_id, $obj->id, $data['academic_year_id']);
            if($obj->id != '' && $obj->id != $complain->user_id && logged_in_user_id() != $obj->id){      
                $message = $this->lang->line('hi'). ' '. $sourse_user->name.',';
                $message .= '<br/><br/>';
                $message .= $this->lang->line('complaint_details')." :";
                $message .= $this->lang->line('complain_user').': ' . $complain_user->name;
                if($complain->role_id == STUDENT)
                {
                    $message .= '<br/>';
                    $message .= $this->lang->line('class').': ' . $complain_user->class_name;
                    $message .= '<br/>';
