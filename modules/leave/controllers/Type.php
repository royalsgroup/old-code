...
class Type extends MY_Controller {
    ...
    * ********************************************************** */
    private function _prepare_type_validation() {
        ...
        $this->form_validation->set_rules('role_id', $this->lang->line('role'), 'trim|required|callback_checkRolesSelected');
    ...
    public function checkRolesSelected($role_id) //custom validation function
    {       
            if ($role_id== "-1" && count($this->input->post('roles[]') )==0)
            {
                    $this->form_validation->set_message('checkRolesSelected', 'Please select Roles');
                    return FALSE;
            }
            else
            {         
