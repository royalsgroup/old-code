                            ...
                            <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                                <thead>
									...
									<tr>
										...
										<td><input type="text" placeholder="Search" style="width:100%;"  class="column_search" /></td>
										<td><input type="text" placeholder="Search" style="width:100%;"  class="column_search" /></td>
										<td><input type="text" id="datepicker" placeholder="Search" style="width:100%;"  class="column_search" /></td>
                                        <td></td>
										
									</tr>
                                </thead>
                                <tbody>   
                                    <?php $count = 1; if(isset($vouchers) && !empty($vouchers)){ ?>
                                        ...
                                        <tr>                                            
                                            ...
                                            <td><?php echo $obj->no_of_entries; ?></td>
											<td><?php if(isset($obj->last_entry_date)){
												echo date('d-m-Y',strtotime($obj->last_entry_date));
											} ?></td>
                                            <td>                                                 
                                                <?php if(has_permission(EDIT, 'accounting', 'vouchers')){ ?>
                                                    <a href="<?php echo site_url('vouchers/edit/'.$obj->id); ?>" class="btn btn-info btn-xs"><i class="fa fa-pencil-square-o"></i> <?php echo $this->lang->line('edit'); ?> </a>
                                                <?php } ?>
												<?php if(has_permission(DELETE, 'accounting', 'vouchers')){ ?>
                                                    <a href="<?php echo site_url('vouchers/delete/'.$obj->id); ?>" onclick="javascript: return confirm('<?php echo $this->lang->line('confirm_alert'); ?>');" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> <?php echo $this->lang->line('delete'); ?> </a>
                                                <?php } ?>
                                               <?php if(has_permission(VIEW, 'accounting', 'vouchers')){ ?>
                                                        <a href="<?php echo site_url('vouchers/view/'.$obj->id); ?>"  class="btn btn-success btn-xs"><i class="fa fa-eye"></i> <?php echo $this->lang->line('view'); ?> </a>
                                                    <?php } 
                                                    ?>
													<?php if(has_permission(ADD, 'accounting', 'accounttransactions')){ ?>
													<?php
                                                    var_dump($iForceAllow.$obj->is_readonly == 0 &&  (!$is_previous_year ||  ($is_previous_year == true && $obj->type == 1)|| $iForceAllow ) );
                                                    if($obj->is_readonly == 0 &&  (!$is_previous_year ||  ($is_previous_year == true && $obj->type == 1)|| $iForceAllow )  ){  
													// check if financial year
													$currentDate = date('Y-m-d');
                                                 
													//if (($currentDate >= $financial_year_start) && ($currentDate <= $financial_year_end))
                                                    { ?>
                                                        <a class="btn btn-info btn-xs" href="<?php echo site_url('transactions/create/'.$obj->id); ?>">New Entry</a>
													<?php }
                                                } } ?>
                                            </td>
                                        </tr>
