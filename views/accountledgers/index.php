...
<?php } else { ?>
									...
									<!--	<th><?php echo $this->lang->line('available_budget'); ?></th>
										...
										<th>Remaining <?php echo $this->lang->line('budget'); ?></th>
							...
							<button type="button" id="load_summery">Load balance</button>
					<?php echo form_close(); ?>							
                            </div>
                        </div>
...
else  if (empty($_POST) && isset($school_info->category) ) 
{
	$category = $school_info->category;
}
 ?>
 var __category = '<?php echo $category; ?>'
 var __balance_loaded =0;
 var __grabd_total_debit = 0;
 var __grabd_total_credit = 0;
...
var edit = false;
	...
	$('#load_summery').on('click', function(){
		var sch_id='<?php print $filter_school_id; ?>';
		if(__balance_loaded =0)
		{
			__balance_loaded =1
		}
		$.ajax({       
            type   : "POST",
            url    : "<?php echo site_url('ajax/get_ledgers_summery'); ?>",
            data   : { school_id:sch_id, category:__category},               
            async  : false,
            success: function(response){                                                   
               ...
               {  
				 ...
				 __grabd_total_debit = response.total_debit;
 				 __grabd_total_credit = response.total_credit;         
				  var page_total_debit = $('.debitTotal').html();   
				  var page_total_credit = $('.creditTotal').html();   
				  $('.debitTotal').html(page_total_debit+'/'+__grabd_total_debit);  
				  $('.creditTotal').html(page_total_credit+'/'+__grabd_total_credit);  
               }
            }
        });		
	})
 ...
 'processing': true,
	  ...
	  footerCallback: function( tfoot, data, start, end, display ) {
		...
		var intVal = function (i) {		
				...
				grand_total_credit = __balance_loaded ? "/"+__grabd_total_credit : "";

					$(api.column(3).footer()).html(
						api.column(3 , { page: 'current' }).data().reduce(function ( a, b ) {
								return intVal(a) + intVal(b);
						}, 0)+''+grand_total_credit
					);
					grand_total_debit = __balance_loaded ? "/"+__grabd_total_debit : "";
