...
class Textlocalsms {
    ...
    public function sendSms($numbers, $message) {
        ...
        // $apiKey = urlencode("ZjQ2MzQwYzMyN2JjOTRlODI3YTNhMGY1MzEzMTJiZGM=");

        // Message details
        $numbers = implode(',', $numbers);
        $sender = urlencode($this->sender_id);
        // $message = "Dear x,%nYour Fee amount x. Has been due as on x, so kindly submit in next 7 working days, VBRNet.";
        $message = rawurlencode($message);
        // var_dump( $sender);
        // die();
        // $username = "info.indiaespectacular@gmail.com";
        // $hash = "ZjQ2MzQwYzMyN2JjOTRlODI3YTNhMGY1MzEzMTJiZGM";
        // $test = "0";

        // Prepare data for POST request
       // $data = array('apikey' => $apiKey, 'numbers' => $numbers, "sender" => $sender, "message" => $message);
        //$data = array('apikey' => $apiKey, 'numbers' => $numbers, "sender" => $sender, "message" => $message);
        //  $data = "apikey=".$hash."&message=".$message."&sender=".$sender."&numbers=".$numbers."&test=".$test;
         $data = array('apikey' => $apiKey, 'numbers' => $numbers, "sender" => $sender, "message" => $message);
        //  var_dump($data);
